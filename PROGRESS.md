...
Status: **Clusters A + B + C all complete (2026-05-18). Cluster C 14-a ✅, 14-b ✅, 14-c ✅. Next is Cluster D (multi-horizon scoring, Stone 10 in code).** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence (see [DECISIONS.md "Constitution tightening v5"](DECISIONS.md) and [CONSTITUTION_V5_PLAN.md](CONSTITUTION_V5_PLAN.md)). Clusters A (Forecast Ledger MVP) and B (calibration shrinkage + Tradable-Edge Action Engine + end-to-end pipeline lock-in) shipped 2026-05-18. Cluster C shipped 2026-05-18: PYRAMID Stone 14 body + FORMULAS Stone 14 entry (14-a); structured `ToyCostModel` + `round_trip_cost_at` + `flat()` classmethod + `src/fingym/evaluator/realized_edge.py` + 20 new unit tests (14-b); integration test at `tests/integration/test_cluster_c_pipeline.py` locks the full chain `toy → forecast → Ledger → shrink → decide → realized_edge` (14-c): BayesianAgent mean realized_edge positive at small size, visibly degraded at large size via sqrt-law; ConfidentAgent and UniformAgent NoActions carry realized_edge = 0 exactly; synthetic monotonicity test confirms sqrt-law signature across deployment sizes. 172 tests green; mypy strict clean across 27 source files; pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster D — multi-horizon scoring (Stone 10 in code).** Cluster C closed 2026-05-18 with the full toy→realized_edge pipeline locked in (sqrt-law fires across deployment sizes; capacity-bucket slicing visible). Cluster D extends the toy to emit realized returns at multiple tick horizons (1m / 3m / 6m / 1y in the toy's tick units) and exercises per-horizon scoreboard scoring: one Contract scored at all configured horizons; per-horizon `realized_edge` columns; per-horizon promotion-gate plumbing (no real promotions yet — that's Cluster G). Build cadence: teach the multi-horizon scoring shape (parallel scoreboard rows per (Contract, horizon) pair vs single row with horizon-indexed columns); distill into PYRAMID Stone 10 body (replaces the pre-v5 Stone 10 summary with the v5 horizon-list framing); implement in `src/fingym/evaluator/scoreboard.py` (new) plus toy horizon emission; verify with the adversarial-agent pipeline that per-horizon realized_edge tracks meaningfully across configured horizons.
...
- **14-b** ✅ (2026-05-18) — `ToyCostModel` replaced with structured fields (`adv`, `spread_bps`, `commission_bps`, `impact_coefficient`, `alpha_decay_bps_per_period`); `__post_init__` validates non-negativity and `adv > 0`; `round_trip_cost_at(notional, horizon_periods) -> float` method computes the total fractional cost via spread + commission + sqrt-law impact + linear alpha decay; `ToyCostModel.flat(round_trip_cost)` classmethod builds a size-independent model for drop-in test compatibility with the Cluster-B-era API. `src/fingym/evaluator/realized_edge.py` added: `realized_edge(action, realized_return, cost_model, horizon_periods) -> float` returns 0 for NoAction and `realized_return × direction − round_trip_cost_at(notional, horizon)` for TradeAction. `action_engine.decide()` takes new `horizon_periods: int = 1` parameter and evaluates the gate cost at `notional_base` (conservative upper bound; documented choice — production iterates). All ~26 existing `ToyCostModel(round_trip_cost=X)` usages migrated via sed to `ToyCostModel.flat(X)` across `tests/unit/test_action_engine.py` and `tests/integration/test_cluster_b_pipeline.py`. 12 new tests at [tests/unit/test_realized_edge.py](tests/unit/test_realized_edge.py) (NoAction = 0, direction sign for long/short, cost subtraction, sqrt-law impact monotonicity, alpha-decay monotonicity, full-decomposition hand-computed value). 8 new ToyCostModel structured-API tests appended to [tests/unit/test_action_engine.py](tests/unit/test_action_engine.py) (flat size-independence, component summation, sqrt-law monotonicity, invalid-input guards on construction and method, decide-with-horizon trade→NoAction shift).
- **14-c** ✅ (2026-05-18) — End-to-end verification at [tests/integration/test_cluster_c_pipeline.py](tests/integration/test_cluster_c_pipeline.py). Five integration tests run the full chain `toy → forecast → Ledger → shrink → decide → realized_edge` over 100 fresh test episodes per agent under a STRUCTURED `ToyCostModel` (adv=$10M, spread=5bp, commission=1bp, impact_coefficient=0.005, alpha_decay=5bp). Verified: BayesianAgent mean realized_edge positive at small size (notional_base=$10k = 0.1% ADV); same agent's mean realized_edge degrades by > 20 bps at large size (notional_base=$10M = 100% ADV) — sqrt-law signature; ConfidentAgent ≥ 90/100 NoActions all carry realized_edge = 0.0 exactly; UniformAgent 100/100 NoActions same; synthetic monotonicity test confirms realized_edge strictly monotone decreasing across deployment sizes from $1k to $10M with a spread > 40 bps end-to-end. Cluster C exit gate cleared.
