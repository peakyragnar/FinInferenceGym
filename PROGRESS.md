...
Status: **Cluster A complete (2026-05-18); Cluster B complete (2026-05-18) — 11c-a ✅, 11c-b ✅, 11d-a ✅, 11d-b ✅, 11d-c ✅. Next is Cluster C (cost models + capacity).** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence (see [DECISIONS.md "Constitution tightening v5"](DECISIONS.md) and [CONSTITUTION_V5_PLAN.md](CONSTITUTION_V5_PLAN.md)). Cluster A (single-believer toy + Forecast Ledger MVP): `src/fingym/ledger/forecast_ledger.py` + 11 unit + 7 integration tests. Cluster B 11c (calibration shrinkage): `src/fingym/action/calibrator.py` (`shrink(raw, sci, ledger, prior_strength)` applies `(n·empirical + k·raw)/(n+k)` per bucket) + 14 unit tests. Cluster B 11d (Tradable-Edge Action Engine): `src/fingym/action/action_engine.py` (`decide` emits `ActionEngineVerdict` with `tradable_edge_score = calibrated_expected_utility − threshold`; TradeAction with fractional-Kelly sizing or NoAction with diagnostic reason) + 23 unit tests; integration test at `tests/integration/test_cluster_b_pipeline.py` locks the three-agent end-to-end crush (Confident ≥ 45/50 NoAction post-shrinkage; Uniform 50/50 NoAction; Bayesian mixed; counterfactual: raw Confident clears the gate, proving Stone 11c is load-bearing). 147 tests green; mypy strict clean across 43 source files (incl. tests); pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster C — cost models + capacity (Stone 14 in toy mode).** Cluster B closed 2026-05-18 with the end-to-end pipeline locked in. Cluster C extends `ToyCostModel` from a single round-trip constant into per-name liquidity + square-root-law impact at deployable size + alpha decay over horizon, and adds the Stone 14 `realized_edge` column to the scoreboard. Build cadence is identical: teach the cost-model decomposition with worked tables, distill into PYRAMID Stone 14 body (replacing the current "v5 reframing pending" marker), then implement under `src/fingym/data/` or `src/fingym/cost/`, then verify against the adversarial-agent pipeline that ConfidentAgent + UniformAgent stay filtered and BayesianAgent's trades carry realistic post-cost edge.
...
- **11d-c** ✅ (2026-05-18) — End-to-end adversarial-agent verification at [tests/integration/test_cluster_b_pipeline.py](tests/integration/test_cluster_b_pipeline.py). Five integration tests run the full pipeline `toy → agent.forecast → calibrator.shrink → action_engine.decide` over 50 fresh test episodes per agent, against a Ledger pre-populated by 100 episodes. Verified: ConfidentAgent ≥ 45/50 NoAction (post-shrinkage crush works); UniformAgent 50/50 NoAction (no edge under threshold = 0.03); BayesianAgent 0 < trades < 50 (mixed; some clear the gate, some don't). One counterfactual locks Stone 11c's role: feeding ConfidentAgent's RAW forecast (bypassing the calibrator) to the engine produces a short with conviction — proving the calibrator is load-bearing for the crush. One shape test verifies all five Contract verification fields are populated and `tradable_edge_score == calibrated_expected_utility − threshold` exactly.
