...
Status: **Clusters A + B + C + D + E + F + G + H all complete (all 2026-05-18). Cluster H H-a ✅, H-b ✅, H-c ✅. Next is Cluster I (Market-State Baseline isolation, Stone 11e in toy) — the final Phase 1 NEW cluster.** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence. Cluster G shipped 2026-05-18 (Stones 39 + 40 distilled; checks 1 + 4 real, checks 2 + 3 stubbed honestly). Cluster H shipped 2026-05-18 (PYRAMID Stone 38 body added + Stone 40 body updated for cross-model): `src/fingym/memory/population.py` (`LlmAgentVariant` + `DEFAULT_VARIANTS` = 2× Haiku (different prompts) + 1× Sonnet + `build_population` factory), `src/fingym/memory/promotion.py` extended with `evaluate_proposal_cross_model` (real check 2 — counts variants where check 1 passes per-variant; ≥`MIN_VARIANTS_PASSING=2` of 3 promotes to L3; ≥1 but < threshold lands in L2; check 3 still stubbed `passed=False` pending Phase 2 NEW), `src/fingym/memory/storage.py` extended with `save_probationary_skill` + `load_probationary_skills` for L2 tier (`memory_registry/probationary/<id>.yaml`), `src/fingym/memory/revalidation.py` (new module — `revalidate(scoreboard, l3_dir, l2_dir)` runs every `REVALIDATION_INTERVAL_ROWS=50` Scoreboard rows; promotes L2 → L3 when subsequent rows tip a second variant, demotes L3 → L2 when variants disagree, retires after `MAX_L2_CYCLES=5` without graduation), `src/fingym/llm/anthropic.py` extended with per-instance `prompt_style: str` field for variant framing. 11 Cluster H tests (9 no-API + 2 live API) verify cross-model gate logic, L2 ↔ L3 transitions, retirement, and population live API. 225 tests green (214 unchanged + 11 Cluster H); mypy strict clean across 35 source files; pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster I — Market-State Baseline isolation (Stone 11e in toy).** Cluster H closed 2026-05-18 with the population + real check 2 + L2 tier + re-validation all wired up. Cluster I is the final Phase 1 NEW cluster: a separate `src/fingym/baseline/` module that reads only toy *headline observables* (simulated rates / volatility / FX-like ticks) and emits its own forecast distribution over realized returns — without seeing the full emission stream the AI Core consumes. The AI Core cannot import from `baseline/` (import-linter rule); the only thing they share is the universe of underlyings and the labelling function. The Scoreboard gains an `incremental_AI_edge` column = (AI realized edge) − (Baseline realized edge). This validates the isolation pattern in toy mode before real headline observables substitute in Phase 2 NEW. After Cluster I, Phase 1 NEW is complete and Phase 2 NEW (real-data substitution) begins.
