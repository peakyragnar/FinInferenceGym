...
Status: **Clusters A + B + C + D all complete (2026-05-18). Cluster D 10-b ✅, 10-c ✅ (10-a deferred — existing pre-v5 PYRAMID Stone 10 body is conceptually correct; only the example table uses pre-v5 `P_AI(state)` vocabulary, scheduled for a v5 vocab cleanup pass alongside Stones 12/13 reframings). Next is Cluster E (PIT discipline + restatements + delisted analogs, Stones 24/26 in toy).** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence (see [DECISIONS.md "Constitution tightening v5"](DECISIONS.md) and [CONSTITUTION_V5_PLAN.md](CONSTITUTION_V5_PLAN.md)). Cluster D shipped 2026-05-18: `realize_returns_at_horizons` added to `src/fingym/toys/synthetic_market.py` (per-horizon independent draws — toy MVP simplification; production would use cumulative correlation); `src/fingym/evaluator/scoreboard.py` added with `ScoreboardRow` frozen dataclass + `Scoreboard` collector (filter_by_horizon / signal_class / agent + mean_brier / log_score / realized_edge aggregations); integration test at `tests/integration/test_cluster_d_pipeline.py` (8 tests) locks per-horizon indexing, per-episode utility monotonicity under alpha-decay, calibrated_expected_return horizon-invariance, Bayesian realized_edge degradation across horizons on consistently-trading episodes, NoAction → realized_edge = 0 at every horizon for Confident and Uniform, brier/log_score populated and bounded per horizon, mean_* aggregation API surface (raises on empty). 180 tests green; mypy strict clean across 28 source files; pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster E — PIT (point-in-time) discipline + restatements + delisted analogs (Stones 24, 26 in toy).** Cluster D closed 2026-05-18. Cluster E extends the toy to simulate restatement events (a `headline_observable` arrives at time `t` but later gets revised at `t+k` with a different value tagged by `as_known`) and delistings (a toy company "delists" mid-trajectory). Adds the `as_known` time-leak guard to the pipeline so an agent training/scoring at time `t` only ever sees data with `as_known ≤ t`. The point: even in synthetic mode, the agent must be unable to peek at a future revision. After Cluster E, the architecture is PIT-disciplined; Phase 2 NEW's real-data ingest will substitute real restatement and delisting feeds into the same plumbing.

Cluster D (✅ 2026-05-18) sub-stones, kept here for the current-cluster pattern:

- **10-a** — Deferred. The pre-v5 PYRAMID Stone 10 body taught the concept faithfully; the only pre-v5 vocabulary remaining is the example table at lines 514–519 (`P_AI(strengthening)` style). Conceptual move (parallel scoreboard rows per (Contract, horizon)) is unchanged under v5. The vocabulary cleanup will happen alongside the Stones 12/13 v5 reframing pass — bundled because all three touch the same vocab.
- **10-b** ✅ (2026-05-18) — Implemented `realize_returns_at_horizons(state, rng, horizons)` in `src/fingym/toys/synthetic_market.py` (independent per-horizon draws; documented MVP simplification vs cumulative-correlation real-world model). Added `src/fingym/evaluator/scoreboard.py` with `ScoreboardRow` frozen dataclass (16 columns spanning agent metadata, raw + calibrated forecasts, Stone-11d engine outputs, realized return / bucket, brier / log_score / realized_edge) and `Scoreboard` collector class with `filter_by_horizon` / `filter_by_signal_class` / `filter_by_agent` + `mean_brier` / `mean_log_score` / `mean_realized_edge` + `horizons_seen` / `total_rows` helpers.
- **10-c** ✅ (2026-05-18) — Integration test at [tests/integration/test_cluster_d_pipeline.py](tests/integration/test_cluster_d_pipeline.py). 8 tests: scoreboard correctly indexes rows by horizon (4 horizons × 50 episodes = 200 rows; `horizons_seen` returns all four); per-episode `calibrated_expected_utility` strictly monotone decreasing in horizon for ALL agents on ALL episodes (deterministic alpha-decay signature); `calibrated_expected_return` is horizon-invariant (forecast same across horizons); BayesianAgent's mean realized_edge at the shortest horizon > the longest on the consistently-trading subset (selection bias eliminated by intersection filtering); ConfidentAgent + UniformAgent NoActions yield realized_edge = 0 at every horizon; brier ∈ [0,2] and log_score ≥ 0 populated per horizon; Scoreboard mean_* aggregations raise on empty row sets.

After Cluster E, Clusters F-I are LLM-agent / memory / population / Market-State Baseline isolation. Sub-stone drafts for each new cluster will be added as we approach them.
