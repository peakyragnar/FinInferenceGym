...
Status: **Clusters A + B + C + D + E + F all complete (all 2026-05-18). Cluster F F-a ✅, F-b ✅, F-c ✅ (5 integration tests auto-skip without `ANTHROPIC_API_KEY`; pass when set). Next is Cluster G (memory + promotion gate, Stones 39 + 40 in toy).** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence. Cluster F shipped 2026-05-18 (PYRAMID Stone 30 distilled into a new Layer 5 section): `src/fingym/llm/contract.py` (typed `ForecastClient` Protocol + `ForecastResponse` frozen dataclass), `src/fingym/llm/anthropic.py` (`AnthropicClient` wrapping the Anthropic SDK — tool-call structured output, prompt caching, default model `claude-haiku-4-5-20251001`), `src/fingym/toys/llm_agent.py` (`LlmAgent` satisfies the `Agent` Protocol — lazy LLM call on first `.forecast` access after new observations), and `tests/conftest.py` (loads `.env` so `ANTHROPIC_API_KEY` is auto-available to tests without manual `export`). 5 integration tests at `tests/integration/test_cluster_f_pipeline.py` (smoke / full pipeline at 10 episodes / forecast-varies-with-signal-stream / cache-invalidates-on-new-observation / NoAction-yields-zero-realized-edge). 194 tests green (194 unchanged Cluster A-E tests + 5 skipped Cluster F tests when no API key); mypy strict clean across 31 source files; pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster G — memory + promotion gate (Stones 39 + 40 in toy).** Cluster F closed 2026-05-18 — LLM cognition layer plugged into the existing verifier without changes to calibrator / Action Engine / realized_edge / Scoreboard. Cluster G builds the layer on top: the LLM agent proposes candidate memory items via `memory_update_proposal` on the Contract; the four-check promotion gate (held-out replay + cross-model regression + survivorship check + domain-of-validity tagging) decides which proposals graduate from L2 (probationary) to L3 (promoted). Promoted skills are read by LLM agents at session start. **To exercise Cluster F's integration tests, set `ANTHROPIC_API_KEY` in `.env` (or as a shell env var) — `tests/conftest.py` auto-loads `.env` so tests just work after the key is set. Without the key, the 5 Cluster F tests skip cleanly.**

Cluster F (✅ 2026-05-18) sub-stones, kept here for the current-cluster pattern:

- **F-a** ✅ (2026-05-18) — Teach + distill. Stone 30 body distilled into PYRAMID as a new **Layer 5 — Model-driven agent on raw evidence** section. Covers: the setup (cognitive engine becomes a model for the first time); three architectural commitments locked by code structure (SDKs isolated to `src/fingym/llm/`; typed Protocol decouples agent from provider; tool-call structured output makes parsing failures structurally impossible); what the model sees (generic-analyst system prompt + emissions wrapped as natural language; no toy-likelihood-table cheating); what the model emits (`submit_forecast(distribution, signal_class_id, thesis_category)`); three properties (model writes its own signal_class_id; calibration shrinkage handles fresh tags gracefully via empty-Ledger pass-through; integration tests are network-dependent and skip without API key); Cluster G/H/I connection forward; in-code pointer.
- **F-b** ✅ (2026-05-18) — Built the LLM module. `src/fingym/llm/contract.py` defines the `ForecastClient` Protocol + `ForecastResponse` frozen dataclass. `src/fingym/llm/anthropic.py` (`AnthropicClient`) wraps the `anthropic` SDK: tool-call structured output via `submit_forecast` tool (forced via `tool_choice`); prompt caching on the system prompt; default model `claude-haiku-4-5-20251001`; reads `ANTHROPIC_API_KEY` from env or accepts an explicit key. `src/fingym/toys/llm_agent.py` (`LlmAgent`) satisfies the `Agent` Protocol — lazy LLM call on first `.forecast` access after new observations; the model's self-applied `signal_class_id` overwrites the agent's default. Type-checked under mypy strict (31 source files).
- **F-c** ✅ (2026-05-18) — Integration test at [tests/integration/test_cluster_f_pipeline.py](tests/integration/test_cluster_f_pipeline.py) (5 tests, auto-skipif no API key). Tests: smoke single-call (verifies SDK plumbing, tool-call output, ForecastResponse parsing); full pipeline at 10 episodes (`toy → forecast → Ledger → shrink → decide → realized_edge` with real model in the cognition layer); forecast-varies-across-signal-streams (all-STRONG vs all-WEAK yields more positive-bucket mass for the bullish case); cache invalidates on new observation (`f1 is f2` until `observe()` is called); NoAction yields zero realized_edge under the LLM. `tests/conftest.py` auto-loads `.env` so the API key is available to pytest without manual export (zero-dep, reads the file directly).

After Cluster G, Clusters H-I are population / Market-State Baseline isolation. Sub-stone drafts for each new cluster will be added as we approach them.
