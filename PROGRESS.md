...
Next: **Stone 19 — Define the model interface contract** (`src/fingym/agents/contract.py`, `src/fingym/agents/contract_validator.py`, `src/fingym/agents/interface.py`). Corresponds to Phase 0 substep 6. Per [CONTRACT.md](CONTRACT.md) MVP spec: pydantic model with required Phase 0 fields (`contract_id`, `decision_time`, `evidence_ids`, `hidden_state_hypotheses`, `ai_belief`, `market_implied_belief`, `belief_delta`, `horizon`, `action_or_no_action`, `recommended_size`, `falsifiers`, `label_plan`, `cognitive_audit_trail`, optional `memory_update_proposal`). Validator enforces applicable Phase 0 checks from CONTRACT.md "Validation" (belief sums to 1, NoAction iff size=0, falsifiers non-empty, cognitive_audit_trail non-empty, etc.). Typed Protocol in `interface.py` (raw evidence in → Contract out). Stub agent wrapping `BayesianAgent` from synthetic-market toy demonstrates Protocol compilation against a concrete implementation. Unit tests in `tests/unit/test_contract.py` and `tests/unit/test_contract_validator.py` covering well-formed and malformed cases.

The remaining substep-4 scoreboard columns (calibration curve / ECE for Stone 8; process-quality flag for Stone 12; decision-quality coherence checks for Stone 13; capacity-adjusted return for Stone 14; multi-horizon scoring infrastructure for Stone 10; action-space-aware expression_type tagging for Stone 11) land as their input machinery ships — columns that need actions wait for Stone 19 / Phase 2; columns that need emissions tables wait for Phase 1.
