...
Status: **Clusters A + B complete; Cluster C 14-a (teach) ✅, 14-b (build) ✅ (all 2026-05-18); next is 14-c (end-to-end realized_edge verification against the adversarial-agent pipeline).** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence (see [DECISIONS.md "Constitution tightening v5"](DECISIONS.md) and [CONSTITUTION_V5_PLAN.md](CONSTITUTION_V5_PLAN.md)). Clusters A (Forecast Ledger MVP) and B (calibration shrinkage + Tradable-Edge Action Engine + end-to-end pipeline lock-in) shipped 2026-05-18. Cluster C 14-a distilled into PYRAMID Stone 14 body + FORMULAS Stone 14 entry. Cluster C 14-b shipped 2026-05-18: `ToyCostModel` replaced with structured `(adv, spread_bps, commission_bps, impact_coefficient, alpha_decay_bps_per_period)` fields plus `round_trip_cost_at(notional, horizon_periods)` method and `flat(round_trip_cost)` classmethod (drop-in for the Cluster-B-era API in tests); `src/fingym/evaluator/realized_edge.py` added with the per-Contract backward-looking computation; `decide()` takes `horizon_periods` parameter and evaluates cost at `notional_base` (conservative upper bound). All ~26 existing ToyCostModel usages migrated via sed to `ToyCostModel.flat(...)`; 12 new realized_edge unit tests + 8 new ToyCostModel structured-API tests. 167 tests green; mypy strict clean across 27 source files; pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster C sub-stone 14-c — end-to-end realized_edge verification.** 14-b is complete (`ToyCostModel` migrated to structured fields; `realized_edge` function added at `src/fingym/evaluator/realized_edge.py`; 20 new tests). 14-c is an integration test that runs the full pipeline `toy → forecast → Ledger → shrink → decide → realized_edge` over many episodes against the three adversarial agents with a STRUCTURED `ToyCostModel` (real components, not flat). Asserts: BayesianAgent's mean realized_edge at small deployable size is positive; at large size (high % of ADV) the same agent's edge degrades visibly via the sqrt-law; ConfidentAgent's NoActions carry `realized_edge = 0` exactly; UniformAgent's NoActions same. After 14-c, Cluster C closes; next cluster is D (multi-horizon scoring, Stone 10 in code).
