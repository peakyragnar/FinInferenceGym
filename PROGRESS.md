...
Status: **Clusters A + B + C + D + E + F + G all complete (all 2026-05-18). Cluster G G-a ✅, G-b ✅, G-c ✅. Next is Cluster H (population variants, Stone 38 + real cross-model regression for Stone 40 check 2).** Phase 0 closed 2026-05-16. Constitution v5 reformulated commitment #2 and restructured the Phase 1 NEW cluster sequence. Cluster F shipped 2026-05-18 (LLM cognition layer, Stone 30; verified end-to-end against live Haiku 4.5 API — 5 integration tests pass with real key). Cluster G shipped 2026-05-18 (PYRAMID Stones 39 + 40 distilled into a new Layer 7 section): `src/fingym/memory/promotion.py` (`Proposal` dataclass + `evaluate_proposal(proposal, scoreboard) -> MemoryArtifact | None`; toy-mode checks 1 + 4 wired up real, checks 2 + 3 stubbed `passed=False` for honest audit), `src/fingym/memory/storage.py` (YAML save/load + `render_for_system_prompt` for L3-injection), `src/fingym/llm/anthropic.py` extended with a second tool `propose_memory_item` (`tool_choice="any"`; mandatory submit_forecast + optional propose), `src/fingym/llm/contract.py` `ForecastResponse.memory_proposal` field, `src/fingym/toys/llm_agent.py` accepts `promoted_skills` at construction and exposes `latest_proposal`. 15 Cluster G tests (13 no-API + 2 live API; auto-skipif no key) at `tests/integration/test_cluster_g_pipeline.py`. 214 tests green (199 unchanged + 15 Cluster G); mypy strict clean across 33 source files; pre-commit 15 hooks clean.
...
Next: **Phase 1 NEW Cluster H — population variants (Stone 38) + real cross-model regression for Stone 40 check 2.** Cluster G closed 2026-05-18 with Stones 39 + 40 wired up in toy mode (checks 1 + 4 real; checks 2 + 3 stubbed). Cluster H runs ≥3 LlmAgent variants in parallel — varying model (Haiku / Sonnet / Opus, or temperature / prompt variant / promoted-skills-subset) — and wires up the real Stone 40 check 2 (cross-model regression): a promoted skill must improve calibration under at least 2 variants, not just one. Cluster H also adds re-validation cycles: as the Scoreboard fills with new trajectories, previously-promoted skills get re-tested; failures demote them back to L2 or retire them. After Cluster H, the only remaining Phase 1 NEW cluster is I (Market-State Baseline isolation, Stone 11e).

Cluster G (✅ 2026-05-18) sub-stones, kept here for the current-cluster pattern:

- **G-a** ✅ (2026-05-18) — Teach + distill. Stones 39 + 40 bodies distilled into PYRAMID as a new **Layer 7 — Memory + promotion gate** section. Stone 39 covers: the OPTIONAL `propose_memory_item` tool call alongside mandatory `submit_forecast`; what the model emits; three properties (proposing is opt-in; model never writes directly to memory — the gate decides; proposal is structured via tool-call). Stone 40 covers: the four checks (held-out replay, cross-model regression, survivorship, domain-of-validity); toy mode wires up checks 1 + 4 with real evaluation and stubs checks 2 + 3 as `passed=False` for honest audit; worked example with concrete Brier numbers; three properties (gate is honest about what it validated; gate decides not the model; promoted artifacts are append-only on disk).
- **G-b** ✅ (2026-05-18) — Built the memory module + LLM extensions. `src/fingym/memory/promotion.py` defines `Proposal` dataclass + `evaluate_proposal(proposal, scoreboard, proposed_at_episode) -> MemoryArtifact | None`. Toy-mode check 1: tag's mean Brier must beat overall by `MIN_CALIBRATION_DELTA = 0.01` with at least `MIN_HELD_OUT_ROWS = 10` rows under the tag. Toy-mode check 4: `signal_class_id` non-empty AND `horizons` non-empty. Checks 2 + 3 stubbed `passed=False` in `PromotionCheckResults`. `src/fingym/memory/storage.py` provides `save_promoted_skill` (L3-only; YAML at `memory_registry/promoted/<id>.yaml`), `load_promoted_skills` (validates via pydantic on load), `render_for_system_prompt` (markdown block injected into LlmAgent system prompt). `src/fingym/llm/anthropic.py` extended: second tool `propose_memory_item(content, signal_class_id, horizons)`; `tool_choice="any"` with system-prompt instruction that submit_forecast is mandatory + propose is optional; both tool calls parsed from the response. `src/fingym/llm/contract.py` adds `ForecastResponse.memory_proposal: Proposal | None`. `src/fingym/toys/llm_agent.py` accepts `promoted_skills: list[MemoryArtifact]` at construction, renders to system prompt, exposes `latest_proposal` for the gate to consume.
- **G-c** ✅ (2026-05-18) — Integration test at [tests/integration/test_cluster_g_pipeline.py](tests/integration/test_cluster_g_pipeline.py) (15 tests: 13 no-API + 2 live API). No-API: promotion gate promotes when signal_class is better-calibrated; rejects when too few held-out rows; rejects when no calibration improvement; rejects on empty signal_class_id; rejects on empty horizons; audit_trail has proposed + promoted entries; checks 2 + 3 stubbed `passed=False` honestly; save+load YAML round-trip preserves nested check results; empty directory returns empty list; missing directory returns empty list; `render_for_system_prompt` handles empty input; rendered output includes content + horizons; save rejects L2 artifacts. Live API (skipif no key): LlmAgent with promoted skills runs and includes them in context; proposal is parsed when emitted (or None — both valid).
