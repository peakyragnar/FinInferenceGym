...
def print_report(
    ...
    print("   [1] Scoreboard — per-agent metrics across all logged decisions.")
    print("       mean_brier: lower is better; range [0, 2] for the 5-bucket")
    print("           forecast space. Compare across agents on the SAME data; a")
    print("           well-calibrated agent's Brier is lower than a bullshitter's.")
    print("       mean_realized_edge: post-cost P&L per Contract. > 0 means the")
    print("           agent makes money after structured frictions (Stone 14).")
    print("       trades vs noact: the gate is filtering if both are non-zero;")
    print("           a NoAction-heavy agent is being conservative.")
    print("   [2] Track C attribution — incremental_AI_edge vs the Market-State")
    print("       Baseline. POSITIVE means the AI beats a simple macro control")
    print("       (real cognitive work). NEAR-ZERO or NEGATIVE means the AI is")
    print("       repackaged macro / bullshit. The most important number here.")
    print("   [3] Memory state — L3 promoted skills + L2 probationary skills.")
    print("       L2 cycles approaching 5 are about to retire (haven't gathered")
    print("       cross-model support after multiple revalidation passes).")
    print("   [4] Recent gate activity — newest-first audit trail. Look for")
    print("       unexpected demotions (skill failed a check on revalidation),")
    print("       retirements (L2 timed out), or surprising promotions.")
    print()
