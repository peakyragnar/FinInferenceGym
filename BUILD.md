...
**Cluster I — Market-State Baseline isolation (Stone 11e in toy) ✅ 2026-05-18.** Toy headline observables (3 bucketed series: rate, vol, FX in {low, mid, high}; `OBSERVABLE_STRENGTH = 0.6` tunable knob) feed a separate `src/fingym/baseline/` module. `MarketStateBaseline` is a hand-coded Bayesian Ledger over `(rate, vol, fx) → realized return bucket` (uniform fallback for empty cells; architectural symmetry with Cluster A's Forecast Ledger). Structural isolation: `mechanisms/lints/no_baseline_imports.py` (new lint, pre-commit hook) blocks any non-Baseline file from importing `fingym.baseline`. The two modules meet only at the Scoreboard, where rows from `agent_id="market_state_baseline"` sit beside the AI's. `Scoreboard.incremental_ai_edge(ai_agent_id, baseline_agent_id)` is the attribution helper. 14 integration tests verify Baseline mechanics + cross-agent attribution + the lint catches violations. After Cluster I, Phase 1 NEW is complete (all 9 clusters ✅); Phase 2 NEW (real-data substitution) is the next major phase.
